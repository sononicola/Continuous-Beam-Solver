...
class Plot:
    def my_plot_style(
        list_of_y_points: list[list] or list, title, x_thicks: list, y_label: str, unit_measure_x: Optional[str] = "m"
    ):
        ...
        ax.set_ylim(
            np.max(list_of_y_points), np.min(list_of_y_points)
        )  # invertiti perché l'asse è invertito
        ax.grid("True")
        ax.axhline(0)
        ...
        ax.set_xticks(
            np.round(x_thicks, 2)
        )  # TODO aggiungere xthick nel massimo in mezzeria
        # ax.set_yticks(np.arange(-300, 250, step=50)) #TODO
        ax.tick_params(axis="both", labelsize=12)
        ax.set_title(title, fontsize=18)
        ax.set_xlabel(f"L [{unit_measure_x}]", fontsize=14)
        ax.set_ylabel(
            y_label, fontsize=14
        )  # TODO aggiungere l'if se si usa per il taglio
        return fig, ax
    ...
    def plot_list_of_y_points(
        s_func: list,
        list_of_y_points: list[list],
        title: str,
        x_thicks: list,
        y_label: str,
        color: str,
