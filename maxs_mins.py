...
def list_of_max_indexes(x: np.array, y:np.array, list_of_points: list) -> list:
    ...
    # Maximum at supports:
    list_of_indexes_supports = [find_local_max_xy(x, y, list_of_points[i] , list_of_points[i])[0] for i in range(len(list_of_points)-1)]
    ...
    list_of_indexes_spans = [find_local_max_xy(x, y, list_of_points[i] , list_of_points[i+1])[0] for i in range(0,len(list_of_points)-2)]
...
def list_of_min_indexes(x: np.array, y:np.array, list_of_points: list) -> list:
    ...
    # Minimum at supports:
    list_of_indexes_supports = [find_local_min_xy(x, y, list_of_points[i] , list_of_points[i])[0] for i in range(len(list_of_points)-1)]
    list_of_indexes_supports[-1] = list_of_indexes_supports[-1] -1  # last index corrisponds of total lenght in arange, but it's out of boundary for x and y 

    # Minimum in the middle of spans: (len -2 because lists are n+1)
    list_of_indexes_spans = [find_local_min_xy(x, y, list_of_points[i] , list_of_points[i+1])[0] for i in range(0,len(list_of_points)-2)]
